...
it('literature 阶段调用 literature_import + search 并产出 analysis', async () => {
  ...
  vi.spyOn(sc, 'graphAlign').mockResolvedValue({ anchors: [{ nodeId: 'ev1', anchor: 'rxn://x' }], source: 'SciGraph' });
...
it('工具失败时抛出（由上层标记 warning）', async () => {
  vi.spyOn(sc, 'literatureImport').mockRejectedValue(new Error('本地服务错误'));
  await expect(runStage('literature', base)).rejects.toThrow(/本地服务错误/);
});
